...
if ( !class_exists("Noanet_Mobile_Menu") ) {
    class Noanet_Mobile_Menu extends Walker_Nav_Menu {
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( isset($item->apus_text_label) && !empty($item->apus_text_label) ) {
                switch ( $item->apus_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'noanet');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'noanet');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Featured', 'noanet');
                        break;
            ...
            $item_output .= '</a>';
            $item_output .= $args->has_children || $apus_mega_profile ? ' <span class="icon-toggle"><i class="fa fa-plus"></i></span>' : '';
